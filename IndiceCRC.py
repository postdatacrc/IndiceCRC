...
if select_seccion=='Índice de calidad':
    ...
    select_servicio=st.radio('Seleccione un servicio para el cálculo del indicador',['Servicios fijos','Servicios móviles'],horizontal=True)
    st.markdown(r"""<hr>""",unsafe_allow_html=True)
    ...
    if select_servicio=='Servicios fijos':
        ...
        with col2:
            select_dimAnalisis=st.selectbox('Tipo de análisis de los indicadores',['Análisis por municipio','Análisis por operador'])
        st.markdown(r"""<hr>""",unsafe_allow_html=True)
        DepartamentosFijos=sorted(BaseFijosMunicipios['departamento'].unique().tolist())
        
        if select_dimAnalisis=='Análisis por municipio':
            col1,col2=st.columns(2)
            with col1:
    ...
    if select_servicio=='Servicios móviles':
        ...
        dict_parametros={'Velocidad de descarga':'download_speed','Velocidad de carga':'upload_speed','Latencia':'latency','Jitter':'jitter','Índice calidad':'Indice_CRC'}
        dict_parametros_unidad={'Velocidad de descarga':'(mpbs)','Velocidad de carga':'(mbps)','Latencia':'(ms)','Jitter':'(ms)','Índice calidad':'(%)'}
        ParametroMovil=st.radio('Elija un parámetro a calcular',['Velocidad de descarga','Velocidad de carga','Latencia','Jitter','Índice calidad'],horizontal=True)        
